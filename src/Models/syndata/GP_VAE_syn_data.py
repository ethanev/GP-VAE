...
def main():
	...
	# 	saver = tf.train.Saver()
		for i in range(steps):
			...
			if i % 500 == 0:
				# for ele in lengths:
				# 	print(ele.eval(feed_dict=inputs))
			...
			if i % 20000 == 0:
				...
				plt.plot(batch_time_steps[0,:seq_0_length],latent[0,:seq_0_length], 'gs')
				plt.plot(batch_time_steps[0,:seq_0_length],latent[1,:seq_0_length], 'bo')
				# plt.plot(batch_time_steps[0,:seq_0_length],latent[2,:seq_0_length], 'go')
				# plt.plot(batch_time_steps[0,:seq_0_length],latent[3,:seq_0_length], 'yo')
				plt.savefig('latent_sample_syn_data_%d.png' %(i))
				plt.gcf().clear()
				length = batch_lengths[0]
				plt.imshow(batch_xs[:length,:].T)
				plt.savefig('input_seq_%d.png' %(i))
				plt.gcf().clear()
				decoded = x_decode.eval(feed_dict=inputs).T
				single_seq = decoded[:,:length]
				plt.imshow(single_seq)
				plt.savefig('decoded_seq_%d.png' %(i))
				plt.gcf().clear()
			# if i % 25000 == 0:
