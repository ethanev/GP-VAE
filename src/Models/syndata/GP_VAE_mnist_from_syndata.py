...
def tf_kernel(sequence, char, number_samples):
	...
	xhar is a of a single number corresponding to the time characteristic of the GP
	'''
	noise = 1e-3
	signal = 1-noise
	...
	# random = tf.ones([tf.shape(L)[0],number_samples]) #THIS IS JUST FOR DEBUGGING!
...
def main():
	...
	logs_path = './GP_VAE_mnist_from_syndata/'
	...
	# 	saver = tf.train.Saver()
		for i in range(steps):
			...
			if i % 500 == 0:
				...
				print('reconstruction loss:',sum_recon_loss.eval(feed_dict=inputs))
				print('GP loss:',sum_gp_kl.eval(feed_dict=inputs))
			# if i % 2000 == 0:
			...
			# 	plt.show()
				...
				# latent = latent_mean.eval(feed_dict=inputs).T
