...
def gp_kl_div(mean, var, inv_K, kernel_mat, det_k, log_var):
	...
	inv_K = tf.cast(inv_K, tf.float64)
	det_k = tf.cast(det_k, tf.float64)
...
def write_file(losses, step, model_type='train'):
	file = open('./GP_prior_models/%s_loss_file_%d.csv' %(model_type, step), 'w')
	for step in losses:
		for ele in step:
			file.write(ele + ',')
		file.write('\n')
...
def main():
	...
	with tf.Session() as sess:
		...
		### uncomment out the next line to train on just 2 batch of 20 to show that the net works
		...
		for i in range(steps):
			...
			if i % 500 == 0:
				...
				step_losses.append(str(sum_recon_loss.eval(feed_dict={x:batch})))
				step_losses.append(str(sum_gp_kl.eval(feed_dict={x:batch})))
			...
			if i % 10000 == 0:
				...
				plt.savefig('./GP_prior_models/input_sequence_%d.png' %(i))
				plt.gcf().clear()
				...
				test_step_losses.append(str(sum_recon_loss.eval(feed_dict={x:batch})))
				test_step_losses.append(str(sum_gp_kl.eval(feed_dict={x:batch})))
				...
				plt.imshow(large_image)
				plt.savefig('./GP_prior_models/output_sequence_%d.png' %(i))
				plt.gcf().clear()
